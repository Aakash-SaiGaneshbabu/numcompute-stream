"""Streaming pipeline abstractions for chaining transformers and estimators."""
...
class Pipeline:
    ...
    Supports fit(), transform(), predict(), and partial_fit().
    """
    ...
    def partial_fit(self, X, y=None):
        ...
        for name, step in self.steps[:-1]:
            if not hasattr(step, "transform"):
                raise ValueError(f"Intermediate step '{name}' must implement transform().")
            self._partial_fit_step(step, Xt, y)
            Xt = step.transform(Xt)
        last_name, last_step = self.steps[-1]
        if hasattr(last_step, "partial_fit"):
            try:
                last_step.partial_fit(Xt, y)
            except TypeError:
                last_step.partial_fit(Xt)
        elif hasattr(last_step, "fit"):
            self._fit_step(last_step, Xt, y)
        else:
